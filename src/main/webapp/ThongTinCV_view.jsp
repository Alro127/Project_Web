					...
					<div class="col-md-4">
						...
						<div class="container text-center mt-4">
							<div class="position-relative d-inline-block">
								<!-- Avatar -->
								<img id="avatarPreview" src="<%=cv.getUngvien().getAvatar()%>"
						...
						<div class="mb-3 personal-info">
							...
							<div class="text-end">
								<button type="button" id="editPersonalInfoBtn"
									class="btn btn-outline-primary ${mode == 'view' ? 'hide-button' : ''}">
									...
									<div class="row mb-2">
										<div class="col-md-6">
											<label class="form-label">Tên trường học</label> <input
												...
												value="<%= cv.getHocVan().get(i).getSchool()%>"
												placeholder="Tên trường học" readonly>
										</div>
										<div class="col-md-6">
											<label class="form-label">Ngành học / Môn học</label> <input
												type="text" name="educationMajor[]" class="form-control"
												value="<%= cv.getHocVan().get(i).getMajor()%>"
												placeholder="Ngành học / Môn học" readonly>
										</div>
									</div>
									<div class="mb-2">
										<label class="form-label">Mô tả quá trình học tập hoặc
											thành tích của bạn</label>
										<textarea name="educationDescription[]" class="form-control"
											readonly rows="2" placeholder="Nhập mô tả..."><%= cv.getHocVan().get(i).getDescription() %></textarea>
									</div>
								</div>
								...
								<div class="row mb-2">
									<div class="col-md-6">
										<label class="form-label">Bắt đầu</label> <input type="date"
											...
											value="<%=  cv.getKinhNghiem().get(i).getStart() %>" readonly>
									</div>
									<div class="col-md-5">
										<label class="form-label">Kết thúc</label> <input type="date"
											name="experienceEnd[]"
											class="form-control me-2 experience-end-date"
											value="<%= cv.getKinhNghiem().get(i).getEnd() %>" readonly>
									</div>
								</div>
								<div class="row mb-2">
									<div class="col-md-6">
										<label class="form-label">Tên công ty</label> <input
											...
											value="<%= cv.getKinhNghiem().get(i).getCompany() %>"
									...
									<div class="col-md-6">
										<label class="form-label">Vị trí công việc</label> <input
											...
											value="<%= cv.getKinhNghiem().get(i).getPosition()  %>"
											placeholder="Vị trí công việc" readonly>
									</div>
								</div>
								<div class="mb-2">
									<label class="form-label">Mô tả kinh nghiệm làm việc</label>
									<textarea name="experienceDescription[]" class="form-control"
										readonly rows="2" placeholder="Nhập mô tả..."><%= cv.getKinhNghiem().get(i).getDescription()  %></textarea>
								</div>
							</div>
							...
							<div id="certificatesContainer">
								...
								<div class="input-group mb-2">
									<input type="text" name="certificates[]" class="form-control"
										...
										value="<%= cv.getChungChi().get(i).getName()  %>" readonly>
								</div>
								<%} %>
							</div>
						</div>
							...
							<div id="skillsContainer">
								...
								<div class="skill-row d-flex align-items-center mb-2">
									<input type="text" name="skills[]" class="form-control me-3"
										...
										value="<%= cv.getKyNang().get(i).getName() %>" readonly>
									...
									<input type="hidden" name="skillLevels[]"
										value="<%= cv.getKyNang().get(i).getLevel() %>">
								</div>
								<%      
				                        }
				                    }
